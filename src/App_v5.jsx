...
// ══════════════════════════════════════════════════════════════════════
// 📁 GITHUB DOCS CONFIG
// Files live in your GitHub repo. Push a PDF/DOCX there and add an
// entry to docs-manifest.json — it appears on the site automatically.
// ══════════════════════════════════════════════════════════════════════
const GITHUB_USER   = "AtifDBA";
const GITHUB_REPO   = "AtifDBA";
const GITHUB_BRANCH = "main";
// Base URL for raw file access (served via jsDelivr CDN — faster, no CORS issues)
const RAW_BASE = `https://cdn.jsdelivr.net/gh/${GITHUB_USER}/${GITHUB_REPO}@${GITHUB_BRANCH}`;
// Manifest URL — JSON file you keep in your repo listing all docs
const MANIFEST_URL = `${RAW_BASE}/docs-manifest.json`;
...
function getSecurityState() {
  try {
    const raw = localStorage.getItem("__sec__");
...
function setSecurityState(s) {
  try { localStorage.setItem("__sec__", JSON.stringify(s)); } catch {}
}
...
// ── STORAGE HELPERS — uses localStorage, works on any browser/host ──
const LS = {
...
async function loadData() {
  try {
    const r = LS.get("itplatform-topics");
    if (r) {
...
function saveData(topics) {
  try { LS.set("itplatform-topics", JSON.stringify(topics)); } catch {}
}
...
export default function App() {
  ...
  // ── DOCS / RESOURCES STATE ──
  const [docs, setDocs]               = useState([]);
  const [docsCategory, setDocsCategory] = useState("all");
  const [docsSearch, setDocsSearch]   = useState("");
  const [docsViewFile, setDocsViewFile] = useState(null);
  ...
  useEffect(() => {
    ...
    // Load custom scripts
    const sr = LS.get("itplatform-scripts");
    if (sr) { try { setCustomScripts(JSON.parse(sr.value)); } catch {} }
    // Load docs from GitHub manifest
    fetch(MANIFEST_URL + "?t=" + Date.now())
      .then(r => r.ok ? r.json() : [])
      .then(manifest => { if (Array.isArray(manifest)) setDocs(manifest); })
      .catch(() => {
        // Fallback: try localStorage cache
        const dr = LS.get("itplatform-docs-cache");
        if (dr) { try { setDocs(JSON.parse(dr.value)); } catch {} }
      });
  ...
  const goDocs    = () => { setView("docs"); setDocsCategory("all"); setDocsSearch(""); setDocsViewFile(null); };
  ...
  const saveScript = (data) => {
    ...
    LS.set("itplatform-scripts", JSON.stringify(next));
  ...
  const saveDocs = (next) => {
    setDocs(next);
    // Cache locally so the list survives a page refresh before next GitHub fetch
    LS.set("itplatform-docs-cache", JSON.stringify(next));
  };
  const saveDoc = (docData) => {
    const existing = docs.find(d => d.id === docData.id);
    const next = existing
      ? docs.map(d => d.id === docData.id ? { ...d, ...docData } : d)
      : [...docs, { ...docData, id: "doc-" + Date.now(), uploadedAt: new Date().toISOString().split("T")[0] }];
    saveDocs(next);
    setAdminView("docs"); setEditingDoc(null);
    showToast("✅ Entry saved! Push docs-manifest.json to GitHub to make it live.");
  };
  const deleteDoc = (id) => {
    saveDocs(docs.filter(d => d.id !== id));
    if (docsViewFile?.id === id) setDocsViewFile(null);
    showToast("🗑️ Removed from list. Update docs-manifest.json in GitHub too.");
  };
    ...
    <div style={{ fontFamily: "'IBM Plex Sans', sans-serif", minHeight: "100vh", background: "#F4F6F9", color: "#0B1220", display: "flex", flexDirection: "column" }}>
      ...
      <nav style={{ position: "sticky", top: 0, zIndex: 100, background: "#0B1220", borderBottom: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 3.5%", height: 60, gap: "1rem" }}>
        ...
        <div style={{ display: "flex", gap: "0.1rem", alignItems: "center" }}>
          {[["Home", goHome], ["About", goAbout], ["Browse All", () => goBrowse()], ["📎 Docs", goDocs]].map(([label, fn]) => (
      ...
      <main style={{ flex: 1 }}>
        ...
        {view === "home" && (
          <div className="fade-in">
            ...
            {/* ── HERO ── */}
            <section style={{ background: "linear-gradient(135deg, #0b1220 0%, #0f2544 60%, #0a1f3a 100%)", padding: "52px 0 48px", position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ position: "absolute", top: "-60px", right: "6%", width: 380, height: 380, background: "radial-gradient(circle, rgba(37,99,235,0.16) 0%, transparent 65%)", pointerEvents: "none" }} />

              <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 5%", position: "relative", display: "flex", gap: "3rem", alignItems: "center", flexWrap: "wrap" }}>
                {/* Left */}
                <div style={{ flex: "1 1 420px", minWidth: 0 }}>
                  <div style={{ display: "inline-flex", alignItems: "center", gap: 7, background: "rgba(37,99,235,0.15)", color: "#60A5FA", border: "1px solid rgba(96,165,250,0.3)", borderRadius: 6, padding: "0.28rem 0.8rem", fontSize: "0.65rem", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: "1.3rem" }}>
                  ...
                  <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(1.8rem, 3vw, 2.75rem)", fontWeight: 800, lineHeight: 1.1, marginBottom: "1rem", color: "#F0F4FF", letterSpacing: "-0.03em" }}>
                  ...
                  <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap", alignItems: "center" }}>
                    <button onClick={() => goBrowse()} className="btn" style={{ padding: "0.72rem 1.5rem", background: "#1D4ED8", color: "#fff", fontSize: "0.87rem", fontWeight: 600, boxShadow: "0 4px 18px rgba(29,78,216,0.4)" }}>
                      Explore Knowledge Base →
                    </button>
                    <button onClick={() => goBrowse("databases")} className="btn" style={{ padding: "0.72rem 1.25rem", background: "rgba(255,255,255,0.07)", color: "#CBD5E1", border: "1px solid rgba(255,255,255,0.13)", fontSize: "0.85rem" }}>
                      🗄️ Database Guides
                    </button>
                  </div>
                </div>
                  ...
                  <div style={{ flex: 1, display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(210px, 1fr))", gap: "0.85rem" }}>
                    {[
                      ...
                      { icon: "🏥", title: "Industry-Specific Understanding", color: "#059669", light: "#ECFDF5",
                        desc: "Real mastery includes healthcare compliance, financial services performance, and e-commerce scaling — with awareness of the regulatory challenges of each industry." },
            ...
            {byCategory.map((cat, ci) => (
              <section key={cat.id} style={{ padding: "40px 0 36px", borderBottom: "1px solid #E1E7EF", background: ci % 2 === 1 ? "#F8FAFC" : "#fff" }}>
                <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 5%" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.4rem", flexWrap: "wrap", gap: "0.8rem" }}>
                    ...
                    <button onClick={() => goBrowse(cat.id)} className="btn" style={{ padding: "0.48rem 1.1rem", background: cat.lightColor, color: cat.color, border: `1px solid ${cat.color}30`, fontSize: "0.8rem", fontWeight: 600, flexShrink: 0 }}>
                      View All {cat.items.length} →
                    </button>
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: "0.9rem" }}>
                    {cat.items.map((t) => (
                      <div key={t.id} onClick={() => goTopic(t)} className="hover-lift card" style={{ padding: "1.25rem", position: "relative", overflow: "hidden", cursor: "pointer" }}>
                ...
                <div style={{ display: "flex", gap: "3rem", alignItems: "flex-start", flexWrap: "wrap" }}>
                  ...
                  <div style={{ flex: 1, display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: "0.75rem" }}>
                    {[
                      ...
                      { icon: "🏗️", title: "Strategic & Operational Support",
                        desc: "Performance optimization, architecture design, migrations, upgrades, and consulting aligned with your business goals." },
                      { icon: "📡", title: "24/7 Monitoring & Proactive Management",
                        desc: "Continuous monitoring catches issues before they impact performance — expert support when problems arise, not just reactive help." },
            ...
            <section style={{ background: "#fff", padding: "32px 0", borderTop: "1px solid #E1E7EF" }}>
              <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 5%" }}>
          ...
          <div className="fade-in">
            <div style={{ background: "#0B1220", padding: "28px 6% 0", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
              ...
              <div style={{ display: "flex", gap: 0 }}>
                {[["topics", "📚 Topics & Pages"], ["scripts", "📜 Script Library"], ["docs", "📎 Documents"]].map(([tab, label]) => (
                  <button key={tab} onClick={() => setAdminView(tab)}
                    style={{ padding: "0.65rem 1.4rem", background: "none", border: "none", borderBottom: adminView === tab || (tab === "topics" && ["edit-topic","edit-page"].includes(adminView)) || (tab === "scripts" && adminView === "edit-script") || (tab === "docs" && adminView === "edit-doc") ? "2px solid #1D4ED8" : "2px solid transparent", color: adminView === tab || (tab === "topics" && ["edit-topic","edit-page"].includes(adminView)) || (tab === "scripts" && adminView === "edit-script") || (tab === "docs" && adminView === "edit-doc") ? "#60A5FA" : "rgba(255,255,255,0.4)", fontFamily: "inherit", fontSize: "0.84rem", fontWeight: 600, cursor: "pointer", transition: "all 0.15s" }}>
                    {label}
                  </button>
                ))}
              </div>
            ...
            {adminView === "docs" && (
              <div style={{ padding: "2rem 6%" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.8rem" }}>
                  <div>
                    <h2 style={{ fontWeight: 700, fontSize: "1.05rem", color: "#0B1220" }}>Documents & Resources ({docs.length})</h2>
                    <p style={{ fontSize: "0.8rem", color: "#64748B", marginTop: "0.2rem" }}>Upload PDF, PPTX, and DOCX files. They appear in the public Docs page for visitors to view and download.</p>
                  </div>
                  <button onClick={() => { setEditingDoc({ title: "", description: "", category: "oracle", data: null, name: "", type: "", size: 0 }); setAdminView("edit-doc"); }} className="btn"
                    style={{ padding: "0.6rem 1.3rem", background: "#1D4ED8", color: "#fff", fontSize: "0.85rem" }}>
                    ＋ Upload Document
                  </button>
                </div>
                ...
                {docs.length === 0 ? (
                  <div style={{ textAlign: "center", padding: "4rem 2rem", border: "2px dashed #E1E7EF", borderRadius: 16, color: "#94A3B8" }}>
                    ...
                    <div style={{ fontWeight: 600, fontSize: "0.95rem", marginBottom: "0.4rem", color: "#64748B" }}>No documents uploaded yet</div>
                    <div style={{ fontSize: "0.82rem", marginBottom: "1.5rem" }}>Upload PDF, PPTX, or DOCX files to share with visitors</div>
                    <button onClick={() => { setEditingDoc({ title: "", description: "", category: "oracle", data: null, name: "", type: "", size: 0 }); setAdminView("edit-doc"); }} className="btn"
                      style={{ padding: "0.65rem 1.4rem", background: "#1D4ED8", color: "#fff", fontSize: "0.85rem" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                    {docs.map(doc => {
                      ...
                      const m = DOC_META[doc.type] || { icon: "📎", label: "FILE", color: "#64748B", light: "#F8FAFC" };
                      const DOC_CATS = { oracle: { label: "Oracle", color: "#C74634" }, postgres: { label: "PostgreSQL", color: "#336791" }, cloud: { label: "Cloud", color: "#0891B2" }, automation: { label: "Automation", color: "#7C3AED" }, mysql: { label: "MySQL", color: "#E8960C" }, sqlserver: { label: "SQL Server", color: "#CC2927" }, general: { label: "General", color: "#374151" } };
                      const cat = DOC_CATS[doc.category] || DOC_CATS.general;
                      const fmtSize = b => b < 1024*1024 ? (b/1024).toFixed(0)+"KB" : (b/1024/1024).toFixed(1)+"MB";
                      return (
                        <div key={doc.id} style={{ background: "#fff", border: "1px solid #E2E2EC", borderRadius: 12, padding: "1rem 1.3rem", display: "flex", alignItems: "center", gap: "1rem" }}>
                          <div style={{ width: 42, height: 42, background: m.light, borderRadius: 9, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.4rem", flexShrink: 0 }}>{m.icon}</div>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.2rem" }}>
                              <span style={{ fontWeight: 700, fontSize: "0.92rem", color: "#0B1220" }}>{doc.title || doc.name}</span>
                              <span style={{ background: m.light, color: m.color, fontSize: "0.62rem", fontWeight: 700, padding: "0.1rem 0.45rem", borderRadius: 4 }}>{m.label}</span>
                              <span style={{ background: "#F1F5F9", color: cat.color, fontSize: "0.62rem", fontWeight: 600, padding: "0.1rem 0.45rem", borderRadius: 4 }}>{cat.label}</span>
                            </div>
                            <div style={{ fontSize: "0.75rem", color: "#64748B" }}>{doc.name} · {fmtSize(doc.size)} · {doc.uploadedAt}</div>
                          </div>
                          <div style={{ display: "flex", gap: "0.5rem", flexShrink: 0 }}>
            ...
            {adminView === "edit-doc" && editingDoc !== null && (
              <DocEditor doc={editingDoc} docs={docs} onSave={saveDoc} onCancel={() => { setAdminView("docs"); setEditingDoc(null); }} />
            )}
        ...
        {view === "docs" && (
          <div className="fade-in">
            {/* Header */}
            <section style={{ background: "linear-gradient(135deg, #0b1220 0%, #0f2544 60%, #0a1f3a 100%)", padding: "44px 6% 40px", position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", inset: 0, backgroundImage: "linear-gradient(rgba(255,255,255,0.022) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.022) 1px, transparent 1px)", backgroundSize: "44px 44px", pointerEvents: "none" }} />
              <div style={{ position: "relative", maxWidth: 1200, margin: "0 auto" }}>
                <div className="section-eyebrow light">Resources Library</div>
                <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(1.5rem, 2.5vw, 2.2rem)", fontWeight: 800, color: "#F0F4FF", letterSpacing: "-0.03em", marginBottom: "0.6rem" }}>
                  📎 Documents & Resources
                </h1>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", marginTop: "1.2rem", flexWrap: "wrap" }}>
                  {[
                    ...
                    { id: "cloud", label: "☁️ Cloud", color: "#0891B2" },
                    { id: "automation", label: "⚙️ Automation", color: "#7C3AED" },
                    { id: "mysql", label: "🐬 MySQL", color: "#E8960C" },
                    { id: "sqlserver", label: "🪟 SQL Server", color: "#CC2927" },
                    { id: "general", label: "📁 General", color: "#64748B" },
                  ].map(cat => (
                    <button key={cat.id} onClick={() => setDocsCategory(cat.id)}
                      style={{ padding: "0.35rem 0.9rem", borderRadius: 20, border: docsCategory === cat.id ? `1.5px solid ${cat.color}` : "1.5px solid rgba(255,255,255,0.12)", background: docsCategory === cat.id ? `${cat.color}22` : "rgba(255,255,255,0.05)", color: docsCategory === cat.id ? cat.color : "rgba(255,255,255,0.55)", fontFamily: "inherit", fontSize: "0.77rem", fontWeight: 600, cursor: "pointer", transition: "all 0.15s" }}>
                      {cat.label} {cat.id !== "all" ? `(${docs.filter(d => d.category === cat.id).length})` : `(${docs.length})`}
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div style={{ maxWidth: 1200, margin: "0 auto", padding: "2rem 6%" }}>
              <input value={docsSearch} onChange={e => setDocsSearch(e.target.value)} placeholder="🔍 Search documents..."
                style={{ width: "100%", padding: "0.75rem 1rem", border: "1.5px solid #E1E7EF", borderRadius: 10, fontSize: "0.9rem", fontFamily: "inherit", outline: "none", background: "#fff", marginBottom: "1.5rem", boxSizing: "border-box" }} />

              {(() => {
                const filtered = docs.filter(d => {
                  const matchCat = docsCategory === "all" || d.category === docsCategory;
                  const q = docsSearch.toLowerCase();
                  const matchQ = !q || (d.title || "").toLowerCase().includes(q) || (d.description || "").toLowerCase().includes(q) || d.name.toLowerCase().includes(q);
                ...
                if (docs.length === 0) return (
                  <div style={{ textAlign: "center", padding: "5rem 2rem", color: "#94A3B8" }}>
                    ...
                    <div style={{ fontWeight: 600, fontSize: "1rem", color: "#64748B", marginBottom: "0.4rem" }}>No documents yet</div>
                    <div style={{ fontSize: "0.85rem" }}>
                      {isAdmin ? <span>Go to <button onClick={() => { setView("admin"); setAdminView("docs"); }} style={{ background: "none", border: "none", color: "#1D4ED8", cursor: "pointer", fontFamily: "inherit", fontSize: "inherit", fontWeight: 600 }}>Admin → Documents</button> to upload files.</span> : "Documents will appear here once uploaded."}
                    </div>
                  </div>
                );
                if (filtered.length === 0) return (
                  <div style={{ textAlign: "center", padding: "3rem", color: "#94A3B8" }}>No documents match your search.</div>
                );
                return (
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "1rem" }}>
                    {filtered.map(doc => {
                      const m = DOC_META[doc.type] || { icon: "📎", label: "FILE", color: "#64748B", light: "#F8FAFC" };
                      return (
                        <div key={doc.id} className="card hover-lift" style={{ padding: "1.3rem", position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                          <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: m.color }} />
                          <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                            <div style={{ width: 44, height: 44, background: m.light, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.4rem", flexShrink: 0 }}>{m.icon}</div>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ fontWeight: 700, fontSize: "0.9rem", color: "#0B1220", lineHeight: 1.3, marginBottom: "0.2rem" }}>{doc.title || doc.name}</div>
                              <div style={{ display: "flex", gap: "0.4rem" }}>
                                <span style={{ background: m.light, color: m.color, fontSize: "0.62rem", fontWeight: 700, padding: "0.1rem 0.45rem", borderRadius: 4 }}>{m.label}</span>
                                <span style={{ background: "#F1F5F9", color: "#475569", fontSize: "0.62rem", fontWeight: 500, padding: "0.1rem 0.45rem", borderRadius: 4 }}>{fmtSize(doc.size)}</span>
                              </div>
                            </div>
                          </div>
                          ...
                          {isAdmin && (
                            <button onClick={() => deleteDoc(doc.id)}
                              style={{ width: "100%", padding: "0.4rem", borderRadius: 6, border: "1px solid #FECACA", background: "#FEF2F2", color: "#DC2626", cursor: "pointer", fontSize: "0.72rem", fontWeight: 600, fontFamily: "inherit" }}>
                              🗑 Delete
                            </button>
                          )}
                        </div>
                ...
                <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 16, overflow: "hidden", width: "92vw", maxWidth: 1060, height: "92vh", display: "flex", flexDirection: "column", boxShadow: "0 25px 80px rgba(0,0,0,0.45)" }}>
                  ...
                  <div style={{ flex: 1, overflow: "hidden", position: "relative" }}>
                    {docsViewFile.type === "application/pdf" ? (
                      <PdfViewer url={docsViewFile.url} data={docsViewFile.data} name={docsViewFile.name || docsViewFile.filename} />
                    ) : (
                      <DocxViewer url={docsViewFile.url} data={docsViewFile.data} name={docsViewFile.name || docsViewFile.filename} />
                    )}
                  </div>
...
// ══════════════════════════════════════════════════════════════════════
// 📄 PDF VIEWER — renders via pdf.js canvas, no download, works publicly
// Converts base64 → Uint8Array → pdf.js renders each page to <canvas>
// ══════════════════════════════════════════════════════════════════════
function PdfViewer({ data, url, name }) {
  ...
  const renderPages = (pdfjsLib, source) => {
    pdfjsLib.getDocument(source).promise
      ...
      .catch(err => { setError("Could not render PDF. " + (err?.message || "")); setLoading(false); });
  };
  ...
  useEffect(() => {
    ...
    loadPdfJs().then(pdfjsLib => {
      if (url) {
        // Load directly from GitHub CDN URL
        renderPages(pdfjsLib, { url });
      } else if (data) {
        // Fallback: base64 path
        const base64 = data.includes(",") ? data.split(",")[1] : data;
        const binary = atob(base64);
        const bytes  = new Uint8Array(binary.length);
        for (let i = 0; i < binary.length; i++) bytes[i] = binary.charCodeAt(i);
        renderPages(pdfjsLib, { data: bytes });
      }
    }).catch(() => { setError("Could not load PDF renderer."); setLoading(false); });
  }, [data, url, scale]);
  ...
  if (loading) return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", gap: "1rem", color: "#64748B" }}>
      ...
      <div style={{ fontWeight: 600 }}>Rendering PDF…</div>
      <div style={{ fontSize: "0.78rem", color: "#94A3B8" }}>Loading pages, please wait</div>
    </div>
  );
  ...
  if (error) return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", gap: "0.8rem", padding: "2rem", textAlign: "center" }}>
      ...
      <div style={{ fontWeight: 600, color: "#374151" }}>Could not render PDF</div>
      <div style={{ fontSize: "0.82rem", color: "#64748B" }}>{error}</div>
    </div>
  );

  return (
    <div ref={containerRef} style={{ height: "100%", overflowY: "auto", background: "#525659", padding: "1.5rem 1rem" }}
      onContextMenu={e => e.preventDefault()}>

      {/* Zoom controls */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "0.75rem", marginBottom: "1rem", position: "sticky", top: 0, zIndex: 10, background: "rgba(82,86,89,0.95)", padding: "0.5rem", borderRadius: 8 }}>
        <button onClick={() => setScale(s => Math.max(0.6, +(s - 0.2).toFixed(1)))}
          style={{ width: 30, height: 30, borderRadius: 6, border: "1px solid rgba(255,255,255,0.2)", background: "rgba(255,255,255,0.1)", color: "#fff", cursor: "pointer", fontSize: "1.1rem", display: "flex", alignItems: "center", justifyContent: "center" }}>−</button>
        <span style={{ color: "#fff", fontSize: "0.8rem", fontFamily: "monospace", minWidth: 44, textAlign: "center" }}>{Math.round(scale * 100)}%</span>
        <button onClick={() => setScale(s => Math.min(3, +(s + 0.2).toFixed(1)))}
          style={{ width: 30, height: 30, borderRadius: 6, border: "1px solid rgba(255,255,255,0.2)", background: "rgba(255,255,255,0.1)", color: "#fff", cursor: "pointer", fontSize: "1.1rem", display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
        <span style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.75rem" }}>{numPages} page{numPages !== 1 ? "s" : ""}</span>
      </div>

      {/* Pages rendered as images — no right-click save */}
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "1rem" }}>
        {pages.map((src, i) => (
          <div key={i} style={{ boxShadow: "0 4px 20px rgba(0,0,0,0.4)", borderRadius: 2 }}>
            <img
              src={src}
              alt={`Page ${i + 1}`}
              draggable="false"
...
// ══════════════════════════════════════════════════════════════════════
function DocxViewer({ data, url, name }) {
  ...
  useEffect(() => {
    ...
    if (url) {
      ...
      fetch(url)
        ...
        .then(buf => loadMammoth(() => convert(buf)))
        .catch(() => { setError("Could not fetch document from GitHub."); setLoading(false); });
    } else if (data) {
      // Fallback: base64
      ...
      const bytes  = new Uint8Array(binary.length);
      for (let i = 0; i < binary.length; i++) bytes[i] = binary.charCodeAt(i);
      loadMammoth(() => convert(bytes.buffer));
    }
  }, [data, url]);
...
// ══════════════════════════════════════════════════════════════════════
// 📎 DOCUMENT ENTRY EDITOR — registers a GitHub-hosted file in manifest
// You push the file to GitHub first, then add its entry here.
// ══════════════════════════════════════════════════════════════════════
function DocEditor({ doc, docs, onSave, onCancel }) {
  const [form, setForm] = useState({
    title:       doc.title       || "",
    description: doc.description || "",
    category:    doc.category    || "oracle",
    filename:    doc.filename    || "",   // exact filename in GitHub repo
  ...
  const rawUrl = form.filename ? `${RAW_BASE}/${encodeURIComponent(form.filename)}` : "";
  const canSave = form.title.trim() && form.filename.trim();

  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "2.5rem 6%" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.5rem" }}>
        <div>
          <h2 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "1.2rem", color: "#0B1220" }}>{doc.id ? "Edit Document Entry" : "Register GitHub Document"}</h2>
          <p style={{ fontSize: "0.78rem", color: "#64748B", marginTop: "0.25rem" }}>File must already be pushed to your GitHub repo first.</p>
        </div>
        <button onClick={onCancel} style={{ background: "#F4F6F9", border: "1px solid #E1E7EF", borderRadius: 8, padding: "0.5rem 1rem", cursor: "pointer", fontSize: "0.82rem", color: "#64748B", fontFamily: "inherit" }}>← Back</button>
      </div>

      {/* How-to strip */}
      <div style={{ background: "#0B1220", borderRadius: 12, padding: "1.2rem 1.5rem", marginBottom: "1.5rem", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
        <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: "0.65rem", color: "#60A5FA", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "0.3rem" }}>Step-by-step</div>
        {[
          `1. Push your PDF/DOCX to: github.com/${GITHUB_USER}/${GITHUB_REPO}`,
          `2. Enter the exact filename below (e.g. MyGuide.pdf)`,
          `3. Save → copy the generated docs-manifest.json → push it to the repo root`,
        ].map((s, i) => (
          <div key={i} style={{ fontSize: "0.8rem", color: "#94A3B8", fontFamily: "'IBM Plex Mono', monospace" }}>{s}</div>
        ))}
      </div>
      ...
      <div style={{ background: "#fff", border: "1px solid #E1E7EF", borderRadius: 16, padding: "2rem", display: "flex", flexDirection: "column", gap: "1.2rem" }}>
        ...
        {/* Filename */}
        <div>
          <label style={{ display: "block", fontSize: "0.75rem", fontWeight: 600, color: "#374151", marginBottom: "0.4rem", letterSpacing: "0.04em", textTransform: "uppercase" }}>Filename in GitHub repo *</label>
          <input value={form.filename} onChange={e => set("filename", e.target.value)}
            placeholder="SELECTWithoutFROMClauseinOracleDatabase23ai_26ai.pdf"
            style={{ width: "100%", padding: "0.75rem 1rem", border: "1.5px solid #E1E7EF", borderRadius: 9, fontSize: "0.85rem", fontFamily: "'IBM Plex Mono', monospace", outline: "none", background: "#F8FAFC", boxSizing: "border-box" }} />
          {rawUrl && (
            <div style={{ marginTop: "0.4rem", fontSize: "0.72rem", color: "#64748B", wordBreak: "break-all" }}>
              Will load from: <span style={{ color: "#1D4ED8" }}>{rawUrl}</span>
            </div>
          )}
        </div>
        ...
        {/* File type */}
        <div>
          <label style={{ display: "block", fontSize: "0.75rem", fontWeight: 600, color: "#374151", marginBottom: "0.4rem", letterSpacing: "0.04em", textTransform: "uppercase" }}>File type *</label>
          <select value={form.type} onChange={e => set("type", e.target.value)} style={{ width: "100%", padding: "0.7rem 1rem", border: "1.5px solid #E1E7EF", borderRadius: 9, fontSize: "0.88rem", background: "#F8FAFC", fontFamily: "inherit", outline: "none" }}>
        ...
        {/* Title */}
        <div>
          <label style={{ display: "block", fontSize: "0.75rem", fontWeight: 600, color: "#374151", marginBottom: "0.4rem", letterSpacing: "0.04em", textTransform: "uppercase" }}>Display Title *</label>
          <input value={form.title} onChange={e => set("title", e.target.value)} placeholder="e.g. SELECT Without FROM in Oracle 23ai"
            style={{ width: "100%", padding: "0.75rem 1rem", border: "1.5px solid #E1E7EF", borderRadius: 9, fontSize: "0.92rem", fontFamily: "inherit", outline: "none", background: "#F8FAFC", boxSizing: "border-box" }} />
        </div>

        {/* Category */}
        <div>
          <label style={{ display: "block", fontSize: "0.75rem", fontWeight: 600, color: "#374151", marginBottom: "0.4rem", letterSpacing: "0.04em", textTransform: "uppercase" }}>Category</label>
          <select value={form.category} onChange={e => set("category", e.target.value)} style={{ width: "100%", padding: "0.7rem 1rem", border: "1.5px solid #E1E7EF", borderRadius: 9, fontSize: "0.88rem", background: "#F8FAFC", fontFamily: "inherit", outline: "none" }}>
        ...
        {/* Description */}
        <div>
          <label style={{ display: "block", fontSize: "0.75rem", fontWeight: 600, color: "#374151", marginBottom: "0.4rem", letterSpacing: "0.04em", textTransform: "uppercase" }}>Description (optional)</label>
          <textarea value={form.description} onChange={e => set("description", e.target.value)}
            placeholder="Brief description of what this document covers..."
            rows={3} style={{ width: "100%", padding: "0.75rem 1rem", border: "1.5px solid #E1E7EF", borderRadius: 9, fontSize: "0.87rem", background: "#F8FAFC", lineHeight: 1.6, fontFamily: "inherit", resize: "vertical", outline: "none", boxSizing: "border-box" }} />
        </div>

        <div style={{ display: "flex", gap: "0.75rem" }}>
          <button onClick={() => canSave && onSave({ ...doc, ...form, url: rawUrl, uploadedAt: doc.uploadedAt || new Date().toISOString().split("T")[0] })} className="btn"
            style={{ flex: 1, padding: "0.85rem", background: canSave ? "#1D4ED8" : "#94A3B8", color: "#fff", fontSize: "0.92rem", cursor: canSave ? "pointer" : "not-allowed", boxShadow: canSave ? "0 4px 16px rgba(29,78,216,0.25)" : "none" }}>
            💾 {doc.id ? "Save Changes" : "Add to Docs List"}
          </button>
          <button onClick={onCancel} className="btn" style={{ flex: 1, padding: "0.85rem", background: "#F4F6F9", color: "#374151", fontSize: "0.92rem", border: "1px solid #E1E7EF" }}>Cancel</button>
        </div>

        {/* Manifest export */}
        {docs && docs.length > 0 && (
          <div style={{ background: "#F8FAFC", border: "1px dashed #CBD5E1", borderRadius: 10, padding: "1rem 1.2rem" }}>
            <div style={{ fontSize: "0.75rem", fontWeight: 600, color: "#374151", marginBottom: "0.5rem", textTransform: "uppercase", letterSpacing: "0.04em" }}>📋 docs-manifest.json — push this to your repo root</div>
            <pre style={{ fontSize: "0.72rem", color: "#374151", fontFamily: "'IBM Plex Mono', monospace", overflowX: "auto", margin: 0, whiteSpace: "pre-wrap", wordBreak: "break-all", maxHeight: 160, overflowY: "auto" }}>
